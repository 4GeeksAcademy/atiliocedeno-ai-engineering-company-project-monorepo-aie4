...
> Esta sección documenta los tipos y utilidades construidos en TypeScript que reflejan este contexto. Cada tipo se alínea con las secciones anteriores del documento.

### `PatientInquiry` — Formulario de consulta

Los 19 campos del formulario están modelados en `src/types/models.ts` como la interfaz `PatientInquiry`:

| Campo del formulario       | Propiedad en TypeScript     | Tipo                       | Validación clave                                          |
| -------------------------- | --------------------------- | -------------------------- | --------------------------------------------------------- |
| Nombre                     | `firstName`                 | `string`                   | Solo letras (con acentos), 2–50 caracteres                |
| Apellido                   | `lastName`                  | `string`                   | Solo letras (con acentos), 2–50 caracteres                |
| Fecha de nacimiento        | `dateOfBirth`               | `string` (ISO date)        | 0–120 años, no futura                                     |
| Correo electrónico         | `email`                     | `string`                   | Formato email válido                                       |
| Número de teléfono         | `phone`                     | `string`                   | Debe empezar con `+` y código de país                      |
| Idioma preferido           | `preferredLanguage`         | `'English' \| 'Spanish'`   | Opciones fijas                                             |
| Clínica preferida          | `preferredClinic`           | `string`                   | Debe ser un nombre de `US_CLINICS`                         |
| Fecha preferida            | `preferredDate`             | `string` (ISO date)        | ≥1 día hábil, ≤60 días                                    |
| Franja horaria             | `preferredTime`             | `'Morning' \| 'Afternoon' \| 'Evening'` | Opciones fijas            |
| Servicio requerido         | `serviceType`               | `ServiceType` (7 opciones) | Opciones exactas del contexto                               |
| Primera visita             | `newPatient`                | `boolean`                  | Obligatorio                                                |
| ¿Tiene seguro?             | `hasInsurance`              | `boolean`                  | Obligatorio                                                |
| Aseguradora                | `insuranceProvider?`        | `string` (opcional)        | Obligatorio si `hasInsurance = true`, máx 100 caracteres  |
| ID de afiliado             | `insuranceMemberId?`        | `string` (opcional)        | Obligatorio si `hasInsurance = true`, 6–20 alfanumérico   |
| Consulta médica            | `healthConcern`             | `string`                   | 20–500 caracteres                                           |
| Consentimiento             | `contactConsent`            | `boolean`                  | Debe ser `true`                                            |
| ID de paciente (recurrente)| `patientId?`                | `string` (opcional)        | Formato `HC-` + 6 alfanuméricos, solo si `newPatient=false` |

Las validaciones correspondientes están en `src/utils/validations.ts`:

- `validatePatientInquiry(inquiry)` — valida los 19 campos completos
- `validateFirstName(value)` — validación individual para tiempo real
- `validateLastName(value)` — validación individual para tiempo real
- `validateInquiryDateOfBirth(value)` — validación individual
- `validateInquiryEmail(value)` — validación individual
- `validateInquiryPhone(value)` — validación individual
- `validateHealthConcern(value)` — validación individual con contador de caracteres
- `validateInsuranceMemberId(value)` — validación individual
- `validatePatientId(value)` — validación individual

### `Clinic` — Datos de clínicas

El modelo `Clinic` incluye los campos `city`, `state` y `openingHours` para reflejar la tabla de Ubicaciones. Las 6 sedes estadounidenses están disponibles como constantes en `US_CLINICS`:

```typescript
export const US_CLINICS: Clinic[] = [
  { id: 'clinic-austin-central',   name: 'HealthCore Austin Central', city: 'Austin',      state: 'TX', phone: '(512) 340-8800', openingHours: 'Lun–Vie 7am–8pm · Sáb 9am–3pm' },
  { id: 'clinic-austin-north',     name: 'HealthCore Austin North',   city: 'Austin',      state: 'TX', phone: '(512) 340-8810', openingHours: 'Lun–Vie 8am–7pm' },
  { id: 'clinic-san-antonio',      name: 'HealthCore San Antonio',    city: 'San Antonio',  state: 'TX', phone: '(210) 720-4400', openingHours: 'Lun–Vie 8am–6pm · Sáb 9am–1pm' },
  { id: 'clinic-miami',            name: 'HealthCore Miami',          city: 'Miami',        state: 'FL', phone: '(305) 510-7700', openingHours: 'Lun–Vie 7am–8pm · Sáb 9am–4pm' },
  { id: 'clinic-orlando',          name: 'HealthCore Orlando',        city: 'Orlando',       state: 'FL', phone: '(407) 892-6600', openingHours: 'Lun–Vie 8am–6pm' },
  { id: 'clinic-atlanta',          name: 'HealthCore Atlanta',        city: 'Atlanta',       state: 'GA', phone: '(404) 330-9900', openingHours: 'Lun–Vie 8am–7pm' },
];
```

Las clínicas del Reino Unido no se incluyen en `US_CLINICS` porque atienden un mercado independiente.